              ...
              <div className={styles.barContainer}>
                ...
                >
                  ...
                  {showPercentage && percentage > 10 && (
                    <span className={styles.barText}>{percentage.toFixed(1)}%</span>
                  )}
                </div>
                {/* Show percentage outside if bar is too narrow */}
                {showPercentage && percentage <= 10 && (
                  <span className={styles.outsideText}>{percentage.toFixed(1)}%</span>
                )}
              </div>

              {/* Right side: Info text (e.g., duration) */}
              {showInfo && item.infoText && (
                <div className={styles.infoText}>{item.infoText}</div>
              )}
            </div>
          )
        })}
